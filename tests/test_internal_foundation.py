...
@foundation_app.get("/__foundation__/auth", dependencies=[Depends(verify_internal_token)])
def foundation_auth_probe():
    return {"ok": True}


@foundation_app.get("/__foundation__/failure", dependencies=[Depends(verify_internal_token)])
def foundation_failure_probe(code: InferenceFailureCode):
    raise InferenceFailure(code)


@foundation_app.get("/__foundation__/llm", dependencies=[Depends(verify_internal_token)])
...
def test_internal_token_unset_fails_closed_with_503(monkeypatch):
    ...
    get_settings.cache_clear()
    response = TestClient(foundation_app).get("/__foundation__/auth")
...
def test_wrong_internal_token_returns_401(foundation_client):
    response = foundation_client.get(
        "/__foundation__/auth", headers={"X-Internal-Token": "wrong-token"}
    )
